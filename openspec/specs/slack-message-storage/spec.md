...
- **THEN** the file SHALL begin with `<!-- slack-inbox -->`, followed by `<!-- workspace: {WORKSPACE_ID} -->` and `<!-- last-sync: {ISO8601_TIMESTAMP} -->` but NO `<!-- hwm:... -->` lines
...
- **THEN** it SHALL return an empty inbox with no messages
