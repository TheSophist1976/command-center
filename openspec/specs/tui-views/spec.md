...
The TUI SHALL support a set of predefined views: Today, All, Weekly, Monthly, Yearly, and NoDueDate. Each view defines a filter predicate applied to the full task list before any user-applied filters. Completed tasks SHALL only appear in the All view; all other views SHALL exclude tasks with status Done.
...
The Today view SHALL show open tasks whose `due_date` equals today's date OR whose `due_date` is `None`. Tasks with a `due_date` in the past or future (other than today) SHALL be excluded.
...
The Weekly view SHALL show open tasks whose `due_date` falls within the current ISO week (Monday through Sunday of the week containing today). Tasks with `due_date` of `None` SHALL be excluded.
...
The Monthly view SHALL show open tasks whose `due_date` falls within the current calendar month (same year and month as today). Tasks with `due_date` of `None` SHALL be excluded.
...
The Yearly view SHALL show open tasks whose `due_date` falls within the current calendar year. Tasks with `due_date` of `None` SHALL be excluded.
